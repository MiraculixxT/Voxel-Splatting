...
void GLChunkRenderer::UploadGrassMesh(int x, int y, const std::vector<float>& vertices) {
    const ChunkCoord coord = {x, y};

    GrassMesh& mesh = m_GrassMeshes[coord];
...
void GLChunkRenderer::RenderGrass(const ViewFrustum &frustum, const int fromX, const int toX, const int fromZ, const int toZ) {
    for (int x = fromX; x <= toX; ++x) {
        for (int z = fromZ; z <= toZ; ++z) {
            const glm::vec3 minBounds(x * CHUNK_WIDTH, 0.0f, z * CHUNK_WIDTH);
            const glm::vec3 maxBounds((x + 1) * CHUNK_WIDTH, CHUNK_HEIGHT, (z + 1) * CHUNK_WIDTH);
            ...
            auto chunk = m_GrassMeshes.find({x, z});
            if (chunk != m_GrassMeshes.end()) {
                const auto& mesh = chunk->second;
                if (mesh.vertexCount > 0) {
                    glBindVertexArray(mesh.VAO);
                    glDrawArrays(GL_TRIANGLES, 0, mesh.vertexCount);
...
void GLChunkRenderer::RenderGrassAll(int fromX, int toX, int fromZ, int toZ)
{
    for (int x = fromX; x <= toX; ++x) {
        for (int z = fromZ; z <= toZ; ++z) {
            auto it = m_GrassMeshes.find({x, z});
